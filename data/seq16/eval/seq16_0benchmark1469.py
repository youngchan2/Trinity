...
def kernel_1(
    ...
    dC_exp = (dC_exp + (tl.dot(temp_5, temp_7) / tl.sum(C_exp, axis=2, dtype=tl.float32)[:, :, None]))
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        tl.store(dWV_ptr + offset_6, tl.dot(temp_20, dV1).to(tl.float32), mask=mask_6)
    # Sequential loop k from 0 to 4544 with tile size BLOCK_K
    for k in range(0, 4544, BLOCK_K):
        ...
        tl.store(dWQ_ptr + offset_8, tl.dot(temp_22, dQ1).to(tl.float32), mask=mask_8)
    # Sequential loop k from 0 to 4544 with tile size BLOCK_K
    for k in range(0, 4544, BLOCK_K):
        ...
        tl.store(dWK_ptr + offset_10, tl.dot(temp_24, dK1).to(tl.float32), mask=mask_10)
