...
def kernel_0(
    ...
    dC_exp = ((tl.dot(temp_12, temp_13) / tl.sum(C_exp, axis=2, dtype=tl.float32)[:, :, None]) + dC_exp)
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        tl.store(dWQ_ptr + offset_5, tl.dot(temp_25, dQ1).to(tl.float32), mask=mask_5)
        temp_26 = tl.trans(temp_24)
        offset_6 = (k + tl.arange(0, BLOCK_K))[:, None] * dWK_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * dWK_stride1
        mask_6 = (k_indices < N)[:, None] & (n_indices < N)[None, :]
        tl.store(dWK_ptr + offset_6, tl.dot(temp_26, dK1).to(tl.float32), mask=mask_6)
        temp_27 = tl.trans(temp_24)
        offset_7 = (k + tl.arange(0, BLOCK_K))[:, None] * dWV_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * dWV_stride1
        mask_7 = (k_indices < N)[:, None] & (n_indices < N)[None, :]
        tl.store(dWV_ptr + offset_7, tl.dot(temp_27, dV1).to(tl.float32), mask=mask_7)
