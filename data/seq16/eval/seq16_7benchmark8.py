...
def kernel_0(
    ...
    dC_sum_ptr,
    dC_sum_stride0: tl.constexpr,
    dC_sum_stride1: tl.constexpr,
    dO2_ptr,
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        Q1 = (tl.dot(temp_0, temp_1).to(tl.float32) + (Q1 * 1).to(tl.float32)).to(tl.float32)
        ...
        K1 = (tl.dot(temp_0, temp_2).to(tl.float32) + (K1 * 1).to(tl.float32)).to(tl.float32)
        ...
        V1 = (tl.dot(temp_0, temp_3).to(tl.float32) + (V1 * 1).to(tl.float32)).to(tl.float32)
    ...
    offset_5 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None] * C_sum_stride0 + (tl.arange(0, 16))[None, :] * C_sum_stride1
    temp_14 = tl.load(C_sum_ptr + offset_5)
    dC_exp = (dC_exp + (tl.dot(temp_12, temp_13).to(tl.float32) / temp_14[:, :, None]).to(tl.float32)).to(tl.float32)
    ...
    temp_16 = tl.load(C_exp_ptr + offset_6)
    temp_17 = tl.expand_dims(temp_10, 1)
    temp_18 = tl.permute(temp_17, (1, 0, 2))
    dV = tl.dot((temp_16 / temp_14[:, :, None]).to(tl.float32), temp_18).to(tl.float32)
    offset_7 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None] * dC_sum_stride0 + (tl.arange(0, 16))[None, :] * dC_sum_stride1
    temp_19 = tl.load(dC_sum_ptr + offset_7)
    dC_exp = (temp_19[:, :, None] + dC_exp).to(tl.float32)
    # Sequential loop k from 0 to 4544 with tile size BLOCK_K
    for k in range(0, 4544, BLOCK_K):
        offset_8 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * C_exp_stride0 + (tl.arange(0, 16))[None, :, None] * C_exp_stride1 + (tl.arange(0, 16))[None, None, :] * C_exp_stride2
        temp_20 = tl.load(C_exp_ptr + offset_8)
        dC = ((dC_exp * temp_20).to(tl.float32) + dC).to(tl.float32)
        dQ = (tl.dot(dC, K).to(tl.float32) + dQ).to(tl.float32)
        temp_21 = tl.permute(dC, (0, 2, 1))
        dK = (tl.dot(temp_21, Q).to(tl.float32) + dK).to(tl.float32)
        temp_22 = tl.permute(dQ, (1, 0, 2))
        # Squeeze dimension 1 from dQ
        temp_23 = tl.reshape(temp_22, (M, D))
        dQ1 = temp_23
        temp_24 = tl.permute(dK, (1, 0, 2))
        # Squeeze dimension 1 from dK
        temp_25 = tl.reshape(temp_24, (M, D))
        dK1 = temp_25
        temp_26 = tl.permute(dV, (1, 0, 2))
        # Squeeze dimension 1 from dV
        temp_27 = tl.reshape(temp_26, (M, D))
        dV1 = temp_27
        offset_9 = (tl.arange(0, 16))[:, None] * X_stride0 + (k + tl.arange(0, BLOCK_K))[None, :] * X_stride1
        k_indices = k + tl.arange(0, BLOCK_K)
        mask_5 = (k_indices < N)[None, :]
        temp_28 = tl.load(X_ptr + offset_9, mask=mask_5, other=0.0)
        temp_29 = tl.trans(temp_28)
        offset_10 = (k + tl.arange(0, BLOCK_K))[:, None] * dWQ_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * dWQ_stride1
        n_indices = n + tl.arange(0, BLOCK_N)
        mask_6 = (k_indices < N)[:, None] & (n_indices < N)[None, :]
        tl.store(dWQ_ptr + offset_10, tl.dot(temp_29, dQ1).to(tl.float32), mask=mask_6)
        temp_30 = tl.trans(temp_28)
        offset_11 = (k + tl.arange(0, BLOCK_K))[:, None] * dWK_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * dWK_stride1
        mask_7 = (k_indices < N)[:, None] & (n_indices < N)[None, :]
        tl.store(dWK_ptr + offset_11, tl.dot(temp_30, dK1).to(tl.float32), mask=mask_7)
        temp_31 = tl.trans(temp_28)
        offset_12 = (k + tl.arange(0, BLOCK_K))[:, None] * dWV_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * dWV_stride1
        mask_8 = (k_indices < N)[:, None] & (n_indices < N)[None, :]
        tl.store(dWV_ptr + offset_12, tl.dot(temp_31, dV1).to(tl.float32), mask=mask_8)


# Metadata for benchmark.py
TENSOR_PARAMS = ['C_exp', 'C_sum', 'WK', 'WQ', 'WV', 'X', 'dC_sum', 'dO2', 'dWK', 'dWQ', 'dWV']
BLOCK_PARAMS = ['block_k']

def forward(C_exp, C_sum, WK, WQ, WV, X, dC_sum, dO2, dWK, dWQ, dWV, block_k=16):
    """
