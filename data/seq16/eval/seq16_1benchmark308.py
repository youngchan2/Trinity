...
def kernel_0(
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        offset_5 = (k + tl.arange(0, BLOCK_K))[:, None] * dWK_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * dWK_stride1
        n_indices = n + tl.arange(0, BLOCK_N)
        mask_5 = (k_indices < N)[:, None] & (n_indices < N)[None, :]
        tl.store(dWK_ptr + offset_5, tl.dot(temp_25, dK1).to(tl.float32), mask=mask_5)
    # Sequential loop k from 0 to 4544 with tile size BLOCK_K
    for k in range(0, 4544, BLOCK_K):
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        tl.store(dWV_ptr + offset_9, tl.dot(temp_29, dV1).to(tl.float32), mask=mask_9)
