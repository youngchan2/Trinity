...
def kernel_0(
    ...
    C_exp = tl.zeros((1, M, M), dtype=tl.float32)
    K = tl.zeros((1, M, D), dtype=tl.float32)
    K1 = tl.zeros((M, BLOCK_N), dtype=tl.float32)
    Q = tl.zeros((1, M, D), dtype=tl.float32)
    Q1 = tl.zeros((M, BLOCK_N), dtype=tl.float32)
    V = tl.zeros((1, M, D), dtype=tl.float32)
    ...
    dV = tl.zeros((1, M, D), dtype=tl.float32)
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        temp_11 = tl.load(dO2_ptr + offset_4, mask=mask_4, other=0.0)
        temp_12 = tl.expand_dims(temp_11, 1)
        temp_13 = tl.permute(temp_12, (1, 0, 2))
        temp_14 = tl.permute(V, (0, 2, 1))
        dC_exp = ((tl.dot(temp_13, temp_14) / tl.sum(C_exp, axis=2, dtype=tl.float32)[:, :, None]) + dC_exp)
        temp_16 = tl.expand_dims(temp_11, 1)
        temp_17 = tl.permute(temp_16, (1, 0, 2))
        dV = (tl.dot(C_exp, temp_17) / tl.sum(C_exp, axis=2, dtype=tl.float32)[:, :, None])
        dC = ((C_exp * dC_exp) + dC)
        dQ = (tl.dot(dC, K) + dQ)
        temp_19 = tl.permute(dC, (0, 2, 1))
        dK = (tl.dot(temp_19, Q) + dK)
        ...
        temp_26 = tl.load(X_ptr + offset_5, mask=mask_5, other=0.0)
        ...
        tl.store(dWQ_ptr + offset_6, tl.dot(temp_27, dQ1).to(tl.float32), mask=mask_6)
        ...
        tl.store(dWK_ptr + offset_7, tl.dot(temp_28, dK1).to(tl.float32), mask=mask_7)
    # Sequential loop k from 0 to 4544 with tile size BLOCK_K
    for k in range(0, 4544, BLOCK_K):
        offset_8 = (tl.arange(0, 16))[:, None] * X_stride0 + (k + tl.arange(0, BLOCK_K))[None, :] * X_stride1
        k_indices = k + tl.arange(0, BLOCK_K)
        mask_8 = (k_indices < N)[None, :]
        temp_29 = tl.load(X_ptr + offset_8, mask=mask_8, other=0.0)
        temp_30 = tl.trans(temp_29)
        offset_9 = (k + tl.arange(0, BLOCK_K))[:, None] * dWV_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * dWV_stride1
        n_indices = n + tl.arange(0, BLOCK_N)
        mask_9 = (k_indices < N)[:, None] & (n_indices < N)[None, :]
        tl.store(dWV_ptr + offset_9, tl.dot(temp_30, dV1).to(tl.float32), mask=mask_9)


# Metadata for benchmark.py
TENSOR_PARAMS = ['WK', 'WQ', 'WV', 'X', 'dO2', 'dWK', 'dWQ', 'dWV']
BLOCK_PARAMS = ['block_k']

def forward(WK, WQ, WV, X, dO2, dWK, dWQ, dWV, block_k=16):
    """
