...
def kernel_0(
    ...
    offset_7 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * Q_stride0 + (tl.arange(0, 16))[None, :, None] * Q_stride1 + (tl.arange(0, 64))[None, None, :] * Q_stride2
    temp_7 = tl.load(Q_ptr + offset_7)
    offset_8 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * K_stride0 + (tl.arange(0, 16))[None, :, None] * K_stride1 + (tl.arange(0, 64))[None, None, :] * K_stride2
    temp_8 = tl.load(K_ptr + offset_8)
    temp_9 = tl.permute(temp_8, (0, 2, 1))
    offset_9 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * C_exp_stride0 + (tl.arange(0, 16))[None, :, None] * C_exp_stride1 + (tl.arange(0, 16))[None, None, :] * C_exp_stride2
    tl.store(C_exp_ptr + offset_9, tl.exp(tl.dot(temp_7, temp_9).to(tl.float32)))
    offset_10 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * C_exp_stride0 + (tl.arange(0, 16))[None, :, None] * C_exp_stride1 + (tl.arange(0, 16))[None, None, :] * C_exp_stride2
    temp_10 = tl.load(C_exp_ptr + offset_10)
    offset_11 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * V_stride0 + (tl.arange(0, 16))[None, :, None] * V_stride1 + (tl.arange(0, 64))[None, None, :] * V_stride2
    temp_11 = tl.load(V_ptr + offset_11)
    offset_12 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * O_stride0 + (tl.arange(0, 16))[None, :, None] * O_stride1 + (tl.arange(0, 64))[None, None, :] * O_stride2
    tl.store(O_ptr + offset_12, tl.dot(temp_10, temp_11))
...
def kernel_1(
    ...
    # Parallel loop n from 0 to 4544 with tile size BLOCK_N
    # Executed across grid dimension 0
    n = 0 + tl.program_id(0) * BLOCK_N
    ...
    offset_1 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None] * C_sum_stride0 + (tl.arange(0, 16))[None, :] * C_sum_stride1
    temp_1 = tl.load(C_sum_ptr + offset_1)
    offset_2 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * O_stride0 + (tl.arange(0, 16))[None, :, None] * O_stride1 + (tl.arange(0, 64))[None, None, :] * O_stride2
    tl.store(O_ptr + offset_2, (temp_0 / temp_1[:, :, None]).to(tl.float32))
...
def kernel_2(
    ...
    offset_0 = (tl.arange(0, 16))[:, None] * dO2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * dO2_stride1
    n_indices = n + tl.arange(0, BLOCK_N)
    mask_4 = (n_indices < N)[None, :]
    temp_0 = tl.load(dO2_ptr + offset_0, mask=mask_4, other=0.0)
    temp_1 = tl.expand_dims(temp_0, 1)
    dO = tl.permute(temp_1, (1, 0, 2))
    offset_1 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None] * C_sum_stride0 + (tl.arange(0, 16))[None, :] * C_sum_stride1
    temp_2 = tl.load(C_sum_ptr + offset_1)
    dO_tmp = (((1 / temp_2[:, :, None]).to(tl.float32) * dO).to(tl.float32) + dO_tmp).to(tl.float32)
    offset_2 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * O_stride0 + (tl.arange(0, 16))[None, :, None] * O_stride1 + (tl.arange(0, 64))[None, None, :] * O_stride2
    temp_3 = tl.load(O_ptr + offset_2)
    dC_sum = (dC_sum + (0 - tl.sum(((temp_3 / temp_2[:, :, None]).to(tl.float32) * dO).to(tl.float32), axis=2, dtype=tl.float32)).to(tl.float32)).to(tl.float32)
    ...
    temp_4 = tl.load(V_ptr + offset_3)
    temp_5 = tl.permute(temp_4, (0, 2, 1))
    dC_exp = (tl.dot(dO_tmp, temp_5).to(tl.float32) + dC_exp).to(tl.float32)
    ...
    temp_6 = tl.load(C_exp_ptr + offset_4)
    temp_7 = tl.permute(temp_6, (0, 2, 1))
    dV = tl.dot(temp_7, dO_tmp).to(tl.float32)
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        dC = ((dC_exp * temp_8).to(tl.float32) + dC).to(tl.float32)
        offset_6 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * K_stride0 + (tl.arange(0, 16))[None, :, None] * K_stride1 + (tl.arange(0, 64))[None, None, :] * K_stride2
        temp_9 = tl.load(K_ptr + offset_6)
        dQ = (tl.dot(dC, temp_9).to(tl.float32) + dQ).to(tl.float32)
        temp_10 = tl.permute(dC, (0, 2, 1))
        offset_7 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * Q_stride0 + (tl.arange(0, 16))[None, :, None] * Q_stride1 + (tl.arange(0, 64))[None, None, :] * Q_stride2
        temp_11 = tl.load(Q_ptr + offset_7)
        dK = (tl.dot(temp_10, temp_11).to(tl.float32) + dK).to(tl.float32)
        temp_12 = tl.permute(dQ, (1, 0, 2))
        # Squeeze dimension 1 from dQ
        temp_13 = tl.reshape(temp_12, (M, D))
        dQ1 = temp_13
        temp_14 = tl.permute(dK, (1, 0, 2))
        # Squeeze dimension 1 from dK
        temp_15 = tl.reshape(temp_14, (M, D))
        dK1 = temp_15
        temp_16 = tl.permute(dV, (1, 0, 2))
        ...
        temp_18 = tl.load(X_ptr + offset_8, mask=mask_5, other=0.0)
        temp_19 = tl.trans(temp_18)
        offset_9 = (k + tl.arange(0, BLOCK_K))[:, None] * dWQ_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * dWQ_stride1
        n_indices = n + tl.arange(0, BLOCK_N)
        mask_6 = (k_indices < N)[:, None] & (n_indices < N)[None, :]
        tl.store(dWQ_ptr + offset_9, tl.dot(temp_19, dQ1).to(tl.float32), mask=mask_6)
        temp_20 = tl.trans(temp_18)
        ...
        tl.store(dWK_ptr + offset_10, tl.dot(temp_20, dK1).to(tl.float32), mask=mask_7)
        temp_21 = tl.trans(temp_18)
        ...
        tl.store(dWV_ptr + offset_11, tl.dot(temp_21, dV1).to(tl.float32), mask=mask_8)


# Metadata for benchmark.py
TENSOR_PARAMS = ['C_exp', 'C_sum', 'K', 'O', 'Q', 'V', 'WK', 'WQ', 'WV', 'X', 'dO2', 'dWK', 'dWQ', 'dWV']
BLOCK_PARAMS = ['block_k']

def forward(C_exp, C_sum, K, O, Q, V, WK, WQ, WV, X, dO2, dWK, dWQ, dWV, block_k=16):
    """
