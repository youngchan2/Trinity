...
def kernel_0(
    O1_ptr,
    O1_stride0: tl.constexpr,
    O1_stride1: tl.constexpr,
    O1_stride2: tl.constexpr,
    WK_ptr,
    ...
    temp_11 = tl.dot((C_exp / tl.sum(C_exp, axis=2, dtype=tl.float32)[:, :, None]), V).to(tl.float32)
    offset_4 = (tl.arange(0, 16))[:, None, None] * O1_stride0 + (((n // BLOCK_N)+tl.arange(0, 1)))[None, :, None] * O1_stride1 + (tl.arange(0, 64))[None, None, :] * O1_stride2
    tl.store(O1_ptr + offset_4, tl.permute(temp_11, (1, 0, 2)))
...
def kernel_1(
    ...
    offset_0 = (tl.arange(0, 16))[:, None, None] * O1_stride0 + (((n // BLOCK_N)+tl.arange(0, 1)))[None, :, None] * O1_stride1 + (tl.arange(0, 64))[None, None, :] * O1_stride2
    temp_0 = tl.load(O1_ptr + offset_0)
    # Squeeze dimension 1 from O1
    temp_1 = tl.reshape(temp_0, (M, D))
    offset_1 = (tl.arange(0, 16))[:, None] * O2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * O2_stride1
    n_indices = n + tl.arange(0, BLOCK_N)
    mask_4 = (n_indices < N)[None, :]
    tl.store(O2_ptr + offset_1, temp_1, mask=mask_4)


# Metadata for benchmark.py
TENSOR_PARAMS = ['O1', 'O2', 'WK', 'WQ', 'WV', 'X']
BLOCK_PARAMS = ['block_k']

def forward(O1, O2, WK, WQ, WV, X, block_k=16):
    """
