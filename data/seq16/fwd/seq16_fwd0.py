...
def kernel_0(
    ...
    temp_11 = tl.dot((C_exp / tl.sum(C_exp, axis=2, dtype=tl.float32)[:, :, None]).to(tl.float32), V).to(tl.float32)
    temp_12 = tl.permute(temp_11, (1, 0, 2))
    # Squeeze dimension 1 (dynamic)
    temp_13 = tl.reshape(temp_12, (temp_12.shape[0], temp_12.shape[2]))
    offset_4 = (tl.arange(0, 16))[:, None] * O2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * O2_stride1
    n_indices = n + tl.arange(0, BLOCK_N)
    mask_4 = (n_indices < N)[None, :]
    tl.store(O2_ptr + offset_4, temp_13.to(tl.float32), mask=mask_4)
