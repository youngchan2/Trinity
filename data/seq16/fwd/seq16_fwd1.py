...
def kernel_0(
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        Q1 = ((Q1 * 1).to(tl.float32) + tl.dot(temp_0, temp_1).to(tl.float32)).to(tl.float32)
        ...
        K1 = ((K1 * 1).to(tl.float32) + tl.dot(temp_0, temp_2).to(tl.float32)).to(tl.float32)
        ...
        V1 = ((V1 * 1).to(tl.float32) + tl.dot(temp_0, temp_3).to(tl.float32)).to(tl.float32)
...
def kernel_1(
    ...
    temp_10 = tl.dot((C_exp / tl.sum(C_exp, axis=2, dtype=tl.float32)[:, :, None]).to(tl.float32), V).to(tl.float32)
    temp_11 = tl.permute(temp_10, (1, 0, 2))
    # Squeeze dimension 1 (dynamic)
    temp_12 = tl.reshape(temp_11, (temp_11.shape[0], temp_11.shape[2]))
    offset_3 = (tl.arange(0, 16))[:, None] * O2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * O2_stride1
    mask_10 = (n_indices < N)[None, :]
    tl.store(O2_ptr + offset_3, temp_12.to(tl.float32), mask=mask_10)
