...
case_file = f"./benchmark_ffn_falcon/ffn_case{num}.txt"
output_file = f"./benchmark_ffn_falcon/ffn_benchmark{num}.py"
...
def start_test():
    ...
    scale_factor = 0.001
    O2 = torch.randn(M, N, dtype=dtype, device=device) * scale_factor
    ...
    WFF1a = torch.randn(N, N, dtype=dtype, device=device) * scale_factor
    WFF1b = torch.randn(N, N, dtype=dtype, device=device) * scale_factor
    WFF2 = torch.randn(N, N, dtype=dtype, device=device) * scale_factor
    WO = torch.randn(N, N, dtype=dtype, device=device) * scale_factor
    X = torch.randn(M, N, dtype=dtype, device=device) * scale_factor
    ...
    FF1a = torch.zeros(M, N, dtype=dtype, device=device)
    FF1b = torch.zeros(M, N, dtype=dtype, device=device)
    FF1b_silu = torch.zeros(M, N, dtype=dtype, device=device)
    FF1 = torch.zeros(M, N, dtype=dtype, device=device)
    ...
    trt_model = TRT_Falcon_Ffn(M, N, WO=WO, WFF1a=WFF1a, WFF1b=WFF1b, WFF2=WFF2).to(device=device)
    trt_model.half()
    ...
    # print("="*50)
    # print("\nComparing results...")
    # if torch.allclose(O_FF_norm, out, rtol=1e-3, atol=1e-4):
    #     print("✓ Results match!")
    # else:
    #     print("✗ Results do not match!")
    #     max_diff = torch.abs(O_FF_norm - out).max()
