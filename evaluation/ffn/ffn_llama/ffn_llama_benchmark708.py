...
def kernel_1(
    ...
    temp_1 = tl.load(attn_O1_ptr + offset_1, mask=mask_4, other=0.0)
...
def kernel_2(
    ...
    for k in range(0, 4096, BLOCK_K):
        ...
        mask_7 = (k_indices < N)[:, None] & (p_indices < P)[None, :]
        temp_1 = tl.load(WFF1b_ptr + offset_1, mask=mask_7, other=0.0)
        FF1b = ((FF1b * 1).to(tl.float16) + tl.dot(temp_0, temp_1).to(tl.float16)).to(tl.float16)
    # Sequential loop k from 0 to 4096 with tile size BLOCK_K
    for k in range(0, 4096, BLOCK_K):
        offset_2 = (tl.arange(0, 16))[:, None] * attn_O2_stride0 + (k + tl.arange(0, BLOCK_K))[None, :] * attn_O2_stride1
        k_indices = k + tl.arange(0, BLOCK_K)
        mask_8 = (k_indices < N)[None, :]
        temp_2 = tl.load(attn_O2_ptr + offset_2, mask=mask_8, other=0.0)
        attn_O3 = (tl.sum((temp_2 * temp_2).to(tl.float16), axis=1, dtype=tl.float16) + (attn_O3 * 1).to(tl.float16)).to(tl.float16)
        offset_3 = (k + tl.arange(0, BLOCK_K))[:, None] * WFF1a_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * WFF1a_stride1
        p_indices = p + tl.arange(0, BLOCK_P)
        mask_9 = (k_indices < N)[:, None] & (p_indices < P)[None, :]
        temp_3 = tl.load(WFF1a_ptr + offset_3, mask=mask_9, other=0.0)
    ...
    p_indices = p + tl.arange(0, BLOCK_P)
    mask_10 = (p_indices < P)[None, :]
    tl.store(FF1_ptr + offset_4, ((FF1b * tl.sigmoid(FF1b.to(tl.float32)).to(tl.float16)).to(tl.float16) * FF1a).to(tl.float16), mask=mask_10)
...
def kernel_3(
    ...
    for p in range(0, 14336, BLOCK_P):
        ...
        p_indices = p + tl.arange(0, BLOCK_P)
        mask_11 = (p_indices < P)[None, :]
        temp_0 = tl.load(FF1_ptr + offset_0, mask=mask_11, other=0.0)
        offset_1 = (p + tl.arange(0, BLOCK_P))[:, None] * WFF2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * WFF2_stride1
        n_indices = n + tl.arange(0, BLOCK_N)
        mask_12 = (p_indices < P)[:, None] & (n_indices < N)[None, :]
        temp_1 = tl.load(WFF2_ptr + offset_1, mask=mask_12, other=0.0)
        FF2 = (tl.dot(temp_0, temp_1).to(tl.float16) + (1 * FF2).to(tl.float16)).to(tl.float16)
    # Store kernel accumulators
    offset_2 = (tl.arange(0, 16))[:, None] * FF2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * FF2_stride1
    n_indices = n + tl.arange(0, BLOCK_N)
    mask_13 = (n_indices < N)[None, :]
    tl.store(FF2_ptr + offset_2, FF2, mask=mask_13)


# Metadata for benchmark.py
TENSOR_PARAMS = ['FF1', 'FF2', 'O2', 'WFF1a', 'WFF1b', 'WFF2', 'WO', 'X', 'attn_O1', 'attn_O2']
BLOCK_PARAMS = ['block_k', 'block_n', 'block_p']

def forward(FF1, FF2, O2, WFF1a, WFF1b, WFF2, WO, X, attn_O1, attn_O2, block_k=16, block_n=16, block_p=16):
    """
