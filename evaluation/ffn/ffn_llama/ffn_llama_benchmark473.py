...
def kernel_2(
    ...
    attn_O_norm = tl.zeros((M, BLOCK_K), dtype=tl.float16)
    ...
    for k in range(0, 4096, BLOCK_K):
        ...
        attn_O_norm = (temp_1 / tl.sqrt((attn_O3 / 4096).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16)
        ...
        temp_2 = tl.load(WFF1a_ptr + offset_2, mask=mask_6, other=0.0)
        FF1a = ((FF1a * 1).to(tl.float16) + tl.dot(attn_O_norm, temp_2).to(tl.float16)).to(tl.float16)
        ...
        FF1b = (tl.dot(attn_O_norm, temp_3).to(tl.float16) + (FF1b * 1).to(tl.float16)).to(tl.float16)
    # Store kernel accumulators
...
def kernel_3(
    ...
    for p in range(0, 14336, BLOCK_P):
        offset_0 = (tl.arange(0, 16))[:, None] * FF1b_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * FF1b_stride1
        p_indices = p + tl.arange(0, BLOCK_P)
        mask_8 = (p_indices < P)[None, :]
        temp_0 = tl.load(FF1b_ptr + offset_0, mask=mask_8, other=0.0)
        offset_1 = (tl.arange(0, 16))[:, None] * FF1a_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * FF1a_stride1
        temp_1 = tl.load(FF1a_ptr + offset_1, mask=mask_8, other=0.0)
        offset_2 = (p + tl.arange(0, BLOCK_P))[:, None] * WFF2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * WFF2_stride1
        n_indices = n + tl.arange(0, BLOCK_N)
        mask_9 = (p_indices < P)[:, None] & (n_indices < N)[None, :]
        temp_2 = tl.load(WFF2_ptr + offset_2, mask=mask_9, other=0.0)
        FF2 = (tl.dot(((temp_0 * tl.sigmoid(temp_0.to(tl.float32)).to(tl.float16)).to(tl.float16) * temp_1).to(tl.float16), temp_2).to(tl.float16) + (FF2 * 1).to(tl.float16)).to(tl.float16)
    # Store kernel accumulators
    offset_3 = (tl.arange(0, 16))[:, None] * FF2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * FF2_stride1
    n_indices = n + tl.arange(0, BLOCK_N)
    mask_10 = (n_indices < N)[None, :]
    tl.store(FF2_ptr + offset_3, FF2, mask=mask_10)
...
def forward(FF1a, FF1b, FF2, O2, WFF1a, WFF1b, WFF2, WO, X, attn_O1, attn_O2, block_k=16, block_n=16, block_p=16):
    ...
    kernel_2[lambda meta: ((14336 - 0 + meta["BLOCK_P"] - 1) // meta["BLOCK_P"],)](
        FF1a,
        FF1a.stride(0),
        FF1a.stride(1),
        FF1b,
    ...
    kernel_3[lambda meta: ((4096 - 0 + meta["BLOCK_N"] - 1) // meta["BLOCK_N"],)](
        FF1a,
        FF1a.stride(0),
        FF1a.stride(1),
        FF1b,
