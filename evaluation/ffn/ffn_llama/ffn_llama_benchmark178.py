...
def kernel_2(
    ...
    for p in range(0, 14336, BLOCK_P):
        ...
        for k in range(0, 4096, BLOCK_K):
            ...
            FF1a = ((FF1a * 1).to(tl.float16) + tl.dot(temp_1, temp_3).to(tl.float16)).to(tl.float16)
    ...
    for p in range(0, 14336, BLOCK_P):
        ...
        temp_5 = tl.load(FF1b_ptr + offset_7, mask=mask_8, other=0.0)
        ...
        temp_6 = tl.load(WFF2_ptr + offset_8, mask=mask_9, other=0.0)
        FF2 = (tl.dot((temp_4 * (tl.sigmoid(temp_5.to(tl.float32)).to(tl.float16) * temp_5).to(tl.float16)).to(tl.float16), temp_6).to(tl.float16) + (FF2 * 1).to(tl.float16)).to(tl.float16)
    # Store kernel accumulators
    offset_9 = (tl.arange(0, 16))[:, None] * FF2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * FF2_stride1
    n_indices = n + tl.arange(0, BLOCK_N)
    mask_10 = (n_indices < N)[None, :]
    tl.store(FF2_ptr + offset_9, FF2, mask=mask_10)
