...
@triton.autotune(
    configs = [
        triton.Config({'BLOCK_N': 32, 'BLOCK_K': 32, 'BLOCK_P': 32}),
        triton.Config({'BLOCK_N': 64, 'BLOCK_K': 64, 'BLOCK_P': 64}),
        triton.Config({'BLOCK_N': 128, 'BLOCK_K': 128, 'BLOCK_P': 128}),
        triton.Config({'BLOCK_N': 32, 'BLOCK_K': 64, 'BLOCK_P': 64}),
        triton.Config({'BLOCK_N': 32, 'BLOCK_K': 128, 'BLOCK_P': 128}),
        triton.Config({'BLOCK_N': 64, 'BLOCK_K': 128, 'BLOCK_P': 128})
    ], key=[]
)
@triton.jit
def kernel_2(
    ...
    for p in range(0, 14336, BLOCK_P):
        ...
        for k in range(0, 4096, BLOCK_K):
            ...
            temp_1 = tl.load(attn_O2_ptr + offset_1, mask=mask_5, other=0.0)
            ...
            FF1b = (tl.dot((temp_1 / tl.sqrt((attn_O3 / 4096).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16), temp_2).to(tl.float16) + (FF1b * 1).to(tl.float16)).to(tl.float16)
            ...
            FF1a = (tl.dot(temp_1, temp_3).to(tl.float16) + (FF1a * 1).to(tl.float16)).to(tl.float16)
        offset_4 = (tl.arange(0, 16))[:, None] * FF1a_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * FF1a_stride1
        p_indices = p + tl.arange(0, BLOCK_P)
        mask_7 = (p_indices < P)[None, :]
        tl.store(FF1a_ptr + offset_4, FF1a, mask=mask_7)
        offset_5 = (tl.arange(0, 16))[:, None] * FF1b_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * FF1b_stride1
        tl.store(FF1b_ptr + offset_5, FF1b, mask=mask_7)
    # Sequential loop p from 0 to 14336 with tile size BLOCK_P
    for p in range(0, 14336, BLOCK_P):
        offset_6 = (tl.arange(0, 16))[:, None] * FF1a_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * FF1a_stride1
        p_indices = p + tl.arange(0, BLOCK_P)
        mask_8 = (p_indices < P)[None, :]
        temp_4 = tl.load(FF1a_ptr + offset_6, mask=mask_8, other=0.0)
        offset_7 = (tl.arange(0, 16))[:, None] * FF1a_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * FF1a_stride1
        tl.store(FF1a_ptr + offset_7, (temp_4 / tl.sqrt((attn_O3 / 4096).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16), mask=mask_8)
        offset_8 = (tl.arange(0, 16))[:, None] * FF1b_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * FF1b_stride1
        temp_5 = tl.load(FF1b_ptr + offset_8, mask=mask_8, other=0.0)
        offset_9 = (p + tl.arange(0, BLOCK_P))[:, None] * WFF2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * WFF2_stride1
        n_indices = n + tl.arange(0, BLOCK_N)
        mask_9 = (p_indices < P)[:, None] & (n_indices < N)[None, :]
        temp_6 = tl.load(WFF2_ptr + offset_9, mask=mask_9, other=0.0)
        FF2 = (tl.dot((temp_4 * (tl.sigmoid(temp_5.to(tl.float32)).to(tl.float16) * temp_5).to(tl.float16)).to(tl.float16), temp_6).to(tl.float16) + (1 * FF2).to(tl.float16)).to(tl.float16)
    # Store kernel accumulators
    offset_10 = (tl.arange(0, 16))[:, None] * FF2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * FF2_stride1
    n_indices = n + tl.arange(0, BLOCK_N)
    mask_10 = (n_indices < N)[None, :]
    tl.store(FF2_ptr + offset_10, FF2, mask=mask_10)
