...
def kernel_0(
    ...
    attn_O1 = tl.zeros((16, BLOCK_N), dtype=tl.float16)
    ...
    tl.store(attn_O1_ptr + offset_2, attn_O1, mask=mask_2)
...
def kernel_1(
    ...
    offset_0 = (tl.arange(0, 16))[:, None] * attn_O1_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * attn_O1_stride1
    n_indices = n + tl.arange(0, BLOCK_N)
    mask_3 = (n_indices < N)[None, :]
    temp_0 = tl.load(attn_O1_ptr + offset_0, mask=mask_3, other=0.0)
    offset_1 = (tl.arange(0, 16))[:, None] * X_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * X_stride1
    temp_1 = tl.load(X_ptr + offset_1, mask=mask_3, other=0.0)
...
def kernel_2(
    ...
    for k in range(0, 4096, BLOCK_K):
        ...
        attn_O3 = (tl.sum((temp_0 * temp_0).to(tl.float16), axis=1, dtype=tl.float16) + (attn_O3 * 1).to(tl.float16)).to(tl.float16)
    # Store kernel accumulators
    offset_1 = (tl.arange(0, 16)) * attn_O3_stride0
    tl.store(attn_O3_ptr + offset_1, attn_O3)
...
def kernel_3(
    ...
    # Initialize kernel accumulators
    FF1a = tl.zeros((16, BLOCK_P), dtype=tl.float16)
    FF1b = tl.zeros((16, BLOCK_P), dtype=tl.float16)
    ...
    for k in range(0, 4096, BLOCK_K):
        ...
        temp_1 = tl.load(WFF1a_ptr + offset_1, mask=mask_6, other=0.0)
        ...
        FF1b = ((FF1b * 1).to(tl.float16) + tl.dot(temp_0, temp_2).to(tl.float16)).to(tl.float16)
    # Store kernel accumulators
    offset_3 = (tl.arange(0, 16))[:, None] * FF1a_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * FF1a_stride1
    p_indices = p + tl.arange(0, BLOCK_P)
    mask_7 = (p_indices < P)[None, :]
    tl.store(FF1a_ptr + offset_3, FF1a, mask=mask_7)
    offset_4 = (tl.arange(0, 16))[:, None] * FF1b_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * FF1b_stride1
    tl.store(FF1b_ptr + offset_4, FF1b, mask=mask_7)
...
def kernel_4(
    ...
    for p in range(0, 14336, BLOCK_P):
        ...
        offset_1 = (tl.arange(0, 16))[:, None] * FF1a_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * FF1a_stride1
        p_indices = p + tl.arange(0, BLOCK_P)
        mask_8 = (p_indices < P)[None, :]
        tl.store(FF1a_ptr + offset_1, (FF1a / tl.sqrt((temp_0 / 4096).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16), mask=mask_8)
        offset_2 = (tl.arange(0, 16))[:, None] * FF1b_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * FF1b_stride1
        tl.store(FF1b_ptr + offset_2, (FF1b / tl.sqrt((temp_0 / 4096).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16), mask=mask_8)
        offset_3 = (p + tl.arange(0, BLOCK_P))[:, None] * WFF2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * WFF2_stride1
        n_indices = n + tl.arange(0, BLOCK_N)
        mask_9 = (p_indices < P)[:, None] & (n_indices < N)[None, :]
        temp_1 = tl.load(WFF2_ptr + offset_3, mask=mask_9, other=0.0)
        FF2 = (tl.dot((FF1a * (tl.sigmoid(FF1b.to(tl.float32)).to(tl.float16) * FF1b).to(tl.float16)).to(tl.float16), temp_1).to(tl.float16) + (FF2 * 1).to(tl.float16)).to(tl.float16)
    # Store kernel accumulators
    offset_4 = (tl.arange(0, 16))[:, None] * FF2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * FF2_stride1
    n_indices = n + tl.arange(0, BLOCK_N)
    mask_10 = (n_indices < N)[None, :]
    tl.store(FF2_ptr + offset_4, FF2, mask=mask_10)


# Metadata for benchmark.py
TENSOR_PARAMS = ['FF1a', 'FF1b', 'FF2', 'O2', 'WFF1a', 'WFF1b', 'WFF2', 'WO', 'X', 'attn_O1', 'attn_O2', 'attn_O3']
BLOCK_PARAMS = ['block_k', 'block_n', 'block_p']

def forward(FF1a, FF1b, FF2, O2, WFF1a, WFF1b, WFF2, WO, X, attn_O1, attn_O2, attn_O3, block_k=16, block_n=16, block_p=16):
    """
    ...
    kernel_3[lambda meta: ((14336 - 0 + meta["BLOCK_P"] - 1) // meta["BLOCK_P"],)](
        FF1a,
        FF1a.stride(0),
        FF1a.stride(1),
        FF1b,
    ...
    kernel_4[lambda meta: ((4096 - 0 + meta["BLOCK_N"] - 1) // meta["BLOCK_N"],)](
        FF1a,
        FF1a.stride(0),
        FF1a.stride(1),
        FF1b,
