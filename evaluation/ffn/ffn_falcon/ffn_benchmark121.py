...
def kernel_1(
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        FF1a = ((1 * FF1a).to(tl.float16) + tl.dot(temp_0, temp_1).to(tl.float16)).to(tl.float16)
    ...
    # Sequential loop k from 0 to 4544 with tile size BLOCK_K
    for k in range(0, 4544, BLOCK_K):
        offset_3 = (tl.arange(0, 16))[:, None] * attn_O2_stride0 + (k + tl.arange(0, BLOCK_K))[None, :] * attn_O2_stride1
        k_indices = k + tl.arange(0, BLOCK_K)
        mask_5 = (k_indices < N)[None, :]
        temp_3 = tl.load(attn_O2_ptr + offset_3, mask=mask_5, other=0.0)
        offset_4 = (tl.arange(0, 16))[:, None] * attn_O_norm_stride0 + (k + tl.arange(0, BLOCK_K))[None, :] * attn_O_norm_stride1
        tl.store(attn_O_norm_ptr + offset_4, (temp_3 / tl.sqrt((attn_O3 / 4544).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16), mask=mask_5)
...
def kernel_2(
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        tl.store(O_FF_ptr + offset_3, (FF2 + temp_2).to(tl.float16), mask=mask_9)
