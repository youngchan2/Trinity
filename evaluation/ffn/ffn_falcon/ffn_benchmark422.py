...
def kernel_0(
    ...
    tl.store(attn_O2_ptr + offset_3, (temp_2 + attn_O1).to(tl.float16))
...
def kernel_1(
    ...
    for n in range(0, 4544, BLOCK_N):
        ...
        tl.store(attn_O3_ptr + offset_2, ((1 * temp_0).to(tl.float16) + tl.sum((temp_1 * temp_1).to(tl.float16), axis=1, dtype=tl.float16)).to(tl.float16))
...
def kernel_2(
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        FF1b = (tl.dot(temp_4, temp_5).to(tl.float16) + (1 * FF1b).to(tl.float16)).to(tl.float16)
    offset_7 = (tl.arange(0, M))[:, None] * FF1_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * FF1_stride1
    tl.store(FF1_ptr + offset_7, ((tl.sigmoid(FF1b.to(tl.float32)).to(tl.float16) * FF1b).to(tl.float16) * FF1a).to(tl.float16))
    # Sequential loop k from 0 to 4544 with tile size BLOCK_K
    for k in range(0, 4544, BLOCK_K):
        ...
        FF2 = (tl.dot(temp_6, temp_7).to(tl.float16) + (1 * FF2).to(tl.float16)).to(tl.float16)
    offset_10 = (tl.arange(0, M))[:, None] * O_FF_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * O_FF_stride1
    tl.store(O_FF_ptr + offset_10, (FF2 + (temp_0 / tl.sqrt((temp_1 / 4544).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16)).to(tl.float16))
...
def kernel_3(
    ...
    for n in range(0, 4544, BLOCK_N):
        ...
        tl.store(O_FF1_ptr + offset_2, (tl.sum((temp_0 * temp_0).to(tl.float16), axis=1, dtype=tl.float16) + (1 * temp_1).to(tl.float16)).to(tl.float16))
