...
def kernel_2(
    ...
    tl.store(attn_O_norm_ptr + offset_2, (temp_0 / tl.sqrt((temp_1 / 4544).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16), mask=mask_4)
...
def kernel_3(
    ...
    # Sequential loop k from 0 to 4544 with tile size BLOCK_K
    for k in range(0, 4544, BLOCK_K):
        offset_0 = (tl.arange(0, 16))[:, None] * attn_O_norm_stride0 + (k + tl.arange(0, BLOCK_K))[None, :] * attn_O_norm_stride1
        k_indices = k + tl.arange(0, BLOCK_K)
        mask_5 = (k_indices < K)[None, :]
        temp_0 = tl.load(attn_O_norm_ptr + offset_0, mask=mask_5, other=0.0)
        offset_1 = (k + tl.arange(0, BLOCK_K))[:, None] * WFF1a_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * WFF1a_stride1
        n_indices = n + tl.arange(0, BLOCK_N)
        mask_6 = (k_indices < K)[:, None] & (n_indices < N)[None, :]
        ...
        offset_3 = (k + tl.arange(0, BLOCK_K))[:, None] * WFF1b_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * WFF1b_stride1
        temp_3 = tl.load(WFF1b_ptr + offset_3, mask=mask_6, other=0.0)
        FF1b = ((FF1b * 1).to(tl.float16) + tl.dot(temp_2, temp_3).to(tl.float16)).to(tl.float16)
    offset_4 = (tl.arange(0, 16)) * attn_O3_stride0
    temp_4 = tl.load(attn_O3_ptr + offset_4)
    FF1b = (FF1b / tl.sqrt((temp_4 / 4544).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16)
    offset_5 = (tl.arange(0, 16))[:, None] * FF1_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * FF1_stride1
    n_indices = n + tl.arange(0, BLOCK_N)
    mask_7 = (n_indices < N)[None, :]
    tl.store(FF1_ptr + offset_5, (FF1a * (FF1b * tl.sigmoid(FF1b.to(tl.float32)).to(tl.float16)).to(tl.float16)).to(tl.float16), mask=mask_7)
...
def kernel_4(
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        tl.store(O_FF_ptr + offset_3, (FF2 + temp_2).to(tl.float16), mask=mask_10)
        offset_4 = (tl.arange(0, 16))[:, None] * O_FF_stride0 + (tl.arange(0, 8192))[None, :] * O_FF_stride1
        fulltile_1_indices = tl.arange(0, 8192)
        mask_11 = (fulltile_1_indices < N)[None, :]
        temp_3 = tl.load(O_FF_ptr + offset_4, mask=mask_11, other=0.0)
        O_FF1 = tl.sum((temp_3 * temp_3).to(tl.float16), axis=1, dtype=tl.float16)
    offset_5 = (tl.arange(0, 16))[:, None] * O_FF_stride0 + (tl.arange(0, 8192))[None, :] * O_FF_stride1
    fulltile_1_indices = tl.arange(0, 8192)
    mask_12 = (fulltile_1_indices < N)[None, :]
    temp_4 = tl.load(O_FF_ptr + offset_5, mask=mask_12, other=0.0)
    offset_6 = (tl.arange(0, 16))[:, None] * O_FF_norm_stride0 + (tl.arange(0, 8192))[None, :] * O_FF_norm_stride1
    tl.store(O_FF_norm_ptr + offset_6, (temp_4 / tl.sqrt((O_FF1 / 4544).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16), mask=mask_12)


# Metadata for benchmark.py
TENSOR_PARAMS = ['FF1', 'O2', 'O_FF', 'O_FF_norm', 'WFF1a', 'WFF1b', 'WFF2', 'WO', 'X', 'attn_O2', 'attn_O3', 'attn_O_norm']
BLOCK_PARAMS = ['block_k', 'block_n']

def forward(FF1, O2, O_FF, O_FF_norm, WFF1a, WFF1b, WFF2, WO, X, attn_O2, attn_O3, attn_O_norm, block_k=16, block_n=16):
    """
