...
def kernel_0(
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        tl.store(attn_O1_ptr + offset_3, (tl.dot(temp_0, temp_1).to(tl.float16) + (temp_2 * 1).to(tl.float16)).to(tl.float16))
...
def kernel_2(
    ...
    for n in range(0, 4544, BLOCK_N):
        ...
        offset_1 = (tl.arange(0, M))[:, None] * attn_O2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * attn_O2_stride1
...
def kernel_3(
    ...
    FF1b_silu = tl.zeros((M, BLOCK_N), dtype=tl.float16)
    ...
    for n in range(0, 4544, BLOCK_N):
        ...
        for k in range(0, 4544, BLOCK_K):
            ...
            FF1a = (tl.dot(temp_2, temp_3).to(tl.float16) + (FF1a * 1).to(tl.float16)).to(tl.float16)
        # Sequential loop k from 0 to 4544 with tile size BLOCK_K
        for k in range(0, 4544, BLOCK_K):
            ...
            FF1b = (tl.dot(temp_4, temp_5).to(tl.float16) + (FF1b * 1).to(tl.float16)).to(tl.float16)
        FF1b_silu = (tl.sigmoid(FF1b.to(tl.float32)).to(tl.float16) * FF1b).to(tl.float16)
        # Sequential loop k from 0 to 4544 with tile size BLOCK_K
        for k in range(0, 4544, BLOCK_K):
            offset_7 = (tl.arange(0, M))[:, None] * FF1_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * FF1_stride1
            tl.store(FF1_ptr + offset_7, (FF1a * FF1b_silu).to(tl.float16))
            ...
            FF2 = ((FF2 * 1).to(tl.float16) + tl.dot(temp_6, temp_7).to(tl.float16)).to(tl.float16)
        offset_10 = (tl.arange(0, M))[:, None] * O_FF_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * O_FF_stride1
        tl.store(O_FF_ptr + offset_10, ((temp_0 / tl.sqrt((temp_1 / 4544).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16) + FF2).to(tl.float16))
        offset_11 = (tl.arange(0, M)) * O_FF1_stride0
        temp_8 = tl.load(O_FF1_ptr + offset_11)
        offset_12 = (tl.arange(0, M)) * O_FF1_stride0
        tl.store(O_FF1_ptr + offset_12, (tl.sum((((temp_0 / tl.sqrt((temp_1 / 4544).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16) + FF2).to(tl.float16) * ((temp_0 / tl.sqrt((temp_1 / 4544).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16) + FF2).to(tl.float16)).to(tl.float16), axis=1, dtype=tl.float16) + (temp_8 * 1).to(tl.float16)).to(tl.float16))
...
TENSOR_PARAMS = ['FF1', 'O2', 'O_FF', 'O_FF1', 'O_FF_norm', 'WFF1a', 'WFF1b', 'WFF2', 'WO', 'X', 'attn_O1', 'attn_O2', 'attn_O3', 'attn_O_norm']
BLOCK_PARAMS = ['block_k', 'block_n']

def forward(FF1, O2, O_FF, O_FF1, O_FF_norm, WFF1a, WFF1b, WFF2, WO, X, attn_O1, attn_O2, attn_O3, attn_O_norm, block_k=16, block_n=16):
    """
