...
def kernel_0(
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        offset_1 = (tl.arange(0, M))[:, None] * O2_stride0 + (k + tl.arange(0, BLOCK_K))[None, :] * O2_stride1
        temp_1 = tl.load(O2_ptr + offset_1)
        offset_2 = (k + tl.arange(0, BLOCK_K))[:, None] * WO_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * WO_stride1
        temp_2 = tl.load(WO_ptr + offset_2)
        offset_3 = (tl.arange(0, M))[:, None] * attn_O1_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * attn_O1_stride1
        tl.store(attn_O1_ptr + offset_3, ((temp_0 * 1).to(tl.float16) + tl.dot(temp_1, temp_2).to(tl.float16)).to(tl.float16))
...
def kernel_1(
    ...
    for n in range(0, 4544, BLOCK_N):
        ...
        tl.store(attn_O3_ptr + offset_4, (tl.sum(((temp_0 + temp_1).to(tl.float16) * (temp_0 + temp_1).to(tl.float16)).to(tl.float16), axis=1, dtype=tl.float16) + (temp_2 * 1).to(tl.float16)).to(tl.float16))
...
def kernel_2(
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        FF1a = ((1 * FF1a).to(tl.float16) + tl.dot(temp_2, temp_3).to(tl.float16)).to(tl.float16)
        offset_5 = (k + tl.arange(0, BLOCK_K))[:, None] * WFF1b_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * WFF1b_stride1
        temp_4 = tl.load(WFF1b_ptr + offset_5)
        FF1b = (tl.dot(temp_2, temp_4).to(tl.float16) + (1 * FF1b).to(tl.float16)).to(tl.float16)
    FF1b_silu = (FF1b * tl.sigmoid(FF1b.to(tl.float32)).to(tl.float16)).to(tl.float16)
    # Sequential loop k from 0 to 4544 with tile size BLOCK_K
    for k in range(0, 4544, BLOCK_K):
        offset_6 = (tl.arange(0, M))[:, None] * FF1_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * FF1_stride1
        tl.store(FF1_ptr + offset_6, (FF1b_silu * FF1a).to(tl.float16))
        offset_7 = (tl.arange(0, M))[:, None] * FF2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * FF2_stride1
        temp_5 = tl.load(FF2_ptr + offset_7)
        ...
        tl.store(FF2_ptr + offset_10, ((temp_5 * 1).to(tl.float16) + tl.dot(temp_6, temp_7).to(tl.float16)).to(tl.float16))
...
def kernel_3(
    ...
    for n in range(0, 4544, BLOCK_N):
        ...
        tl.store(O_FF1_ptr + offset_4, ((1 * temp_2).to(tl.float16) + tl.sum(((temp_0 + temp_1).to(tl.float16) * (temp_0 + temp_1).to(tl.float16)).to(tl.float16), axis=1, dtype=tl.float16)).to(tl.float16))
