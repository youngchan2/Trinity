...
def kernel_1(
    ...
    # Sequential loop k from 0 to 4544 with tile size BLOCK_K
    for k in range(0, 4544, BLOCK_K):
        offset_0 = (tl.arange(0, M)) * O_FF1_stride0
        temp_0 = tl.load(O_FF1_ptr + offset_0)
        offset_1 = (tl.arange(0, M))[:, None] * O_FF_stride0 + (k + tl.arange(0, BLOCK_K))[None, :] * O_FF_stride1
        temp_1 = tl.load(O_FF_ptr + offset_1)
        offset_2 = (tl.arange(0, M)) * O_FF1_stride0
        tl.store(O_FF1_ptr + offset_2, ((temp_0 * 1).to(tl.float16) + tl.sum((temp_1 * temp_1).to(tl.float16), axis=1, dtype=tl.float16)).to(tl.float16))
...
TENSOR_PARAMS = ['FF1', 'O_FF', 'O_FF1', 'O_FF_norm', 'WFF2', 'attn_O_norm']
BLOCK_PARAMS = ['block_k', 'block_n']

def forward(FF1, O_FF, O_FF1, O_FF_norm, WFF2, attn_O_norm, block_k=16, block_n=16):
    """
