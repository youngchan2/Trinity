...
def kernel_1(
    ...
    tl.store(FF1_ptr + offset_3, ((tl.sigmoid(FF1b.to(tl.float32)).to(tl.float16) * FF1b).to(tl.float16) * FF1a).to(tl.float16), mask=mask_5)
...
def kernel_2(
    ...
    # Sequential loop p from 0 to 4096 with tile size BLOCK_P
    for p in range(0, 14336, BLOCK_P):
        offset_0 = (tl.arange(0, 16))[:, None] * FF1_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * FF1_stride1
        p_indices = p + tl.arange(0, BLOCK_P)
        mask_6 = (p_indices < P)[None, :]
        temp_0 = tl.load(FF1_ptr + offset_0, mask=mask_6, other=0.0)
        offset_1 = (p + tl.arange(0, BLOCK_P))[:, None] * WFF2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * WFF2_stride1
        n_indices = n + tl.arange(0, BLOCK_N)
        mask_7 = (p_indices < P)[:, None] & (n_indices < N)[None, :]
        temp_1 = tl.load(WFF2_ptr + offset_1, mask=mask_7, other=0.0)
    ...
    tl.store(FF2_ptr + offset_2, FF2, mask=mask_8)
