...
def kernel_3(
    ...
    tl.store(O_FF_norm_ptr + offset_1, (temp_0 / tl.sqrt((O_FF1 / 4544).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16), mask=mask_10)


# Metadata for benchmark.py
TENSOR_PARAMS = ['FF1', 'O2', 'O_FF', 'O_FF_norm', 'WFF1a', 'WFF1b', 'WFF2', 'WO', 'X', 'attn_O2', 'attn_O_norm']
BLOCK_PARAMS = ['block_k', 'block_n']

def forward(FF1, O2, O_FF, O_FF_norm, WFF1a, WFF1b, WFF2, WO, X, attn_O2, attn_O_norm, block_k=16, block_n=16):
    """
