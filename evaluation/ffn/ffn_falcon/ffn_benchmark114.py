...
def kernel_0(
    ...
    tl.store(attn_O2_ptr + offset_3, (temp_2 + attn_O1).to(tl.float16), mask=mask_2)
...
def kernel_1(
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        tl.store(attn_O_norm_ptr + offset_2, (temp_1 / tl.sqrt((attn_O3 / 4544).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16), mask=mask_4)
        offset_3 = (k + tl.arange(0, BLOCK_K))[:, None] * WFF1a_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * WFF1a_stride1
        n_indices = n + tl.arange(0, BLOCK_N)
        mask_5 = (k_indices < K)[:, None] & (n_indices < N)[None, :]
        temp_2 = tl.load(WFF1a_ptr + offset_3, mask=mask_5, other=0.0)
        FF1a = (tl.dot(temp_1, temp_2).to(tl.float16) + (1 * FF1a).to(tl.float16)).to(tl.float16)
        offset_4 = (k + tl.arange(0, BLOCK_K))[:, None] * WFF1b_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * WFF1b_stride1
        temp_3 = tl.load(WFF1b_ptr + offset_4, mask=mask_5, other=0.0)
        FF1b = ((1 * FF1b).to(tl.float16) + tl.dot(temp_1, temp_3).to(tl.float16)).to(tl.float16)
    ...
    tl.store(FF1_ptr + offset_5, ((FF1b * tl.sigmoid(FF1b.to(tl.float32)).to(tl.float16)).to(tl.float16) * FF1a).to(tl.float16), mask=mask_6)
...
def kernel_3(
    ...
    mask_10 = (fulltile_1_indices < N)[None, :]
    temp_0 = tl.load(O_FF_ptr + offset_0, mask=mask_10, other=0.0)
    offset_1 = (tl.arange(0, 16)) * O_FF1_stride0
    tl.store(O_FF1_ptr + offset_1, tl.sum((temp_0 * temp_0).to(tl.float16), axis=1, dtype=tl.float16))
...
def kernel_4(
    ...
    # Parallel loop k from 0 to 4544 with tile size BLOCK_K
    # Executed across grid dimension 0
    ...
    offset_2 = (tl.arange(0, 16))[:, None] * O_FF_norm_stride0 + (tl.arange(0, 8192))[None, :] * O_FF_norm_stride1
...
TENSOR_PARAMS = ['FF1', 'O2', 'O_FF', 'O_FF1', 'O_FF_norm', 'WFF1a', 'WFF1b', 'WFF2', 'WO', 'X', 'attn_O2', 'attn_O_norm']
BLOCK_PARAMS = ['block_k', 'block_n']

def forward(FF1, O2, O_FF, O_FF1, O_FF_norm, WFF1a, WFF1b, WFF2, WO, X, attn_O2, attn_O_norm, block_k=16, block_n=16):
    """
