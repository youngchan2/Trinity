...
def kernel_2(
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        FF1b = ((FF1b * 1).to(tl.float16) + tl.dot(temp_2, temp_4).to(tl.float16)).to(tl.float16)
    offset_6 = (tl.arange(0, M))[:, None] * FF1_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * FF1_stride1
    tl.store(FF1_ptr + offset_6, ((FF1b * tl.sigmoid(FF1b.to(tl.float32)).to(tl.float16)).to(tl.float16) * FF1a).to(tl.float16))
    # Sequential loop k from 0 to 4544 with tile size BLOCK_K
    for k in range(0, 4544, BLOCK_K):
        offset_7 = (tl.arange(0, M))[:, None] * FF1_stride0 + (k + tl.arange(0, BLOCK_K))[None, :] * FF1_stride1
        temp_5 = tl.load(FF1_ptr + offset_7)
        offset_8 = (k + tl.arange(0, BLOCK_K))[:, None] * WFF2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * WFF2_stride1
        temp_6 = tl.load(WFF2_ptr + offset_8)
        FF2 = (tl.dot(temp_5, temp_6).to(tl.float16) + (FF2 * 1).to(tl.float16)).to(tl.float16)
    offset_9 = (tl.arange(0, M))[:, None] * O_FF_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * O_FF_stride1
    tl.store(O_FF_ptr + offset_9, ((temp_0 / tl.sqrt((temp_1 / 4544).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16) + FF2).to(tl.float16))
...
def kernel_3(
    ...
    for n in range(0, 4544, BLOCK_N):
        ...
        offset_2 = (tl.arange(0, M)) * O_FF1_stride0
        tl.store(O_FF1_ptr + offset_2, (tl.sum((temp_0 * temp_0).to(tl.float16), axis=1, dtype=tl.float16) + (temp_1 * 1).to(tl.float16)).to(tl.float16))
