...
def kernel_2(
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        FF1a = ((1 * FF1a).to(tl.float16) + tl.dot((temp_0 / tl.sqrt((temp_1 / 4544).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16), temp_2).to(tl.float16)).to(tl.float16)
        ...
        FF1b = (tl.dot(temp_0, temp_3).to(tl.float16) + (1 * FF1b).to(tl.float16)).to(tl.float16)
    ...
    tl.store(FF1_ptr + offset_6, (FF1a * (tl.sigmoid(FF1b.to(tl.float32)).to(tl.float16) * FF1b).to(tl.float16)).to(tl.float16), mask=mask_6)
...
def kernel_4(
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        O_FF1 = ((1 * O_FF1).to(tl.float16) + tl.sum((temp_0 * temp_0).to(tl.float16), axis=1, dtype=tl.float16)).to(tl.float16)
    # Store kernel accumulators
