...
def kernel_0(
    ...
    offset_8 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None] * C_sum_stride0 + (tl.arange(0, 16))[None, :] * C_sum_stride1
    tl.store(C_sum_ptr + offset_8, C_sum.to(tl.float16))
    offset_9 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * O_stride0 + (tl.arange(0, 16))[None, :, None] * O_stride1 + (tl.arange(0, 64))[None, None, :] * O_stride2
    tl.store(O_ptr + offset_9, O.to(tl.float16))
...
def kernel_1(
    ...
    tl.store(O_ptr + offset_2, (temp_0 / temp_1[:, :, None]).to(tl.float16))
    temp_2 = tl.permute(temp_0, (1, 0, 2))
    offset_3 = (tl.arange(0, 16))[:, None] * O2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * O2_stride1
    n_indices = n + tl.arange(0, BLOCK_N)
    mask_6 = (n_indices < N)[None, :]
    tl.store(O2_ptr + offset_3, tl.reshape(temp_2, (M, D)).to(tl.float16), mask=mask_6)


# Metadata for benchmark.py
TENSOR_PARAMS = ['C_sum', 'K_cache', 'O', 'O2', 'V_cache', 'WK', 'WQ', 'WV', 'X']
BLOCK_PARAMS = ['block_k', 'block_p']

def forward(C_sum, K_cache, O, O2, V_cache, WK, WQ, WV, X, block_k=16, block_p=16):
    """
