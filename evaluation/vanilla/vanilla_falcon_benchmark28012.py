...
def kernel_0(
    ...
    O = tl.zeros((1, 16, 64), dtype=tl.float32)
    ...
    for p in range(0, 1040, BLOCK_P):
        ...
        temp_10 = tl.load(V_cache_ptr + offset_9, mask=mask_7, other=0.0)
        O = ((O * 1) + tl.dot(temp_9, temp_10.to(tl.float32)))
    # Store kernel accumulators
    offset_10 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * O_stride0 + (tl.arange(0, 16))[None, :, None] * O_stride1 + (tl.arange(0, 64))[None, None, :] * O_stride2
    tl.store(O_ptr + offset_10, O.to(tl.float16))
...
def kernel_1(
    ...
    # Sequential loop p from 0 to 1040 with tile size BLOCK_P
    for p in range(0, 1040, BLOCK_P):
        offset_0 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * C_exp_stride0 + (tl.arange(0, 16))[None, :, None] * C_exp_stride1 + (p + tl.arange(0, BLOCK_P))[None, None, :] * C_exp_stride2
        p_indices = p + tl.arange(0, BLOCK_P)
        mask_8 = (p_indices < P+M)[None, None, :]
        temp_0 = tl.load(C_exp_ptr + offset_0, mask=mask_8, other=0.0)
        C_sum = ((1 * C_sum).to(tl.float16) + tl.sum(temp_0, axis=2)).to(tl.float16)
    # Sequential loop p from 0 to 1040 with tile size BLOCK_P
    for p in range(0, 1040, BLOCK_P):
        offset_1 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * O_stride0 + (tl.arange(0, 16))[None, :, None] * O_stride1 + (tl.arange(0, 64))[None, None, :] * O_stride2
        temp_1 = tl.load(O_ptr + offset_1)
        ...
        tl.store(O_ptr + offset_2, (temp_1 / C_sum[:, :, None]).to(tl.float16))
    ...
    tl.store(O2_ptr + offset_4, tl.reshape(temp_3, (M, D)).to(tl.float16), mask=mask_9)


# Metadata for benchmark.py
TENSOR_PARAMS = ['C_exp', 'K_cache', 'O', 'O2', 'V_cache', 'WK', 'WQ', 'WV', 'X']
BLOCK_PARAMS = ['block_k', 'block_p']

def forward(C_exp, K_cache, O, O2, V_cache, WK, WQ, WV, X, block_k=16, block_p=16):
    """
