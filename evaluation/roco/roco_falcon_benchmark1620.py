...
def kernel_0(
    ...
    for p in range(0, 1040, BLOCK_P):
        ...
        C_sum = (tl.sum(temp_9, axis=2) + (C_sum * 1))
    # Sequential loop p from 0 to 1040 with tile size BLOCK_P
    for p in range(0, 1040, BLOCK_P):
        ...
        tl.store(C_out2_ptr + offset_12, tl.sum((C_div * C_div), axis=1, dtype=tl.float16), mask=mask_10)
    ...
    tl.store(O2_ptr + offset_13, tl.reshape(temp_12, (M, D)), mask=mask_11)
