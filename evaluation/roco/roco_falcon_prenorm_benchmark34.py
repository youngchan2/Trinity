...
def kernel_0(
    ...
    C_div = tl.zeros((1, M, BLOCK_P), dtype=tl.float16)
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        X2 = (tl.sum((temp_0 * temp_0).to(tl.float16), axis=1, dtype=tl.float16) + (X2 * 1).to(tl.float16)).to(tl.float16)
    # Sequential loop k from 0 to 4544 with tile size BLOCK_K
    for k in range(0, 4544, BLOCK_K):
        ...
        Q1 = ((Q1 * 1).to(tl.float16) + tl.dot((temp_1 / tl.sqrt((X2 / 4544).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16), temp_2).to(tl.float16)).to(tl.float16)
        ...
        K1 = ((K1 * 1).to(tl.float16) + tl.dot((temp_1 / tl.sqrt((X2 / 4544).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16), temp_3).to(tl.float16)).to(tl.float16)
        offset_4 = (k + tl.arange(0, BLOCK_K))[:, None] * WV_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * WV_stride1
        mask_4 = (k_indices < N)[:, None] & (n_indices < N)[None, :]
        temp_4 = tl.load(WV_ptr + offset_4, mask=mask_4, other=0.0)
        V1 = ((V1 * 1).to(tl.float16) + tl.dot((temp_1 / tl.sqrt((X2 / 4544).to(tl.float16).to(tl.float32)).to(tl.float16)[:, None]).to(tl.float16), temp_4).to(tl.float16)).to(tl.float16)
    ...
    offset_5 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * K_cache_stride0 + (1024 + tl.arange(0, 16))[None, :, None] * K_cache_stride1 + (tl.arange(0, 64))[None, None, :] * K_cache_stride2
    tl.store(K_cache_ptr + offset_5, K)
    offset_6 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * V_cache_stride0 + (1024 + tl.arange(0, 16))[None, :, None] * V_cache_stride1 + (tl.arange(0, 64))[None, None, :] * V_cache_stride2
    tl.store(V_cache_ptr + offset_6, V)
    # Sequential loop p from 0 to 1040 with tile size BLOCK_P
    for p in range(0, 1040, BLOCK_P):
        offset_7 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * K_cache_stride0 + (p + tl.arange(0, BLOCK_P))[None, :, None] * K_cache_stride1 + (tl.arange(0, 64))[None, None, :] * K_cache_stride2
        p_indices = p + tl.arange(0, BLOCK_P)
        mask_5 = (p_indices < P+M)[None, :, None]
        temp_8 = tl.load(K_cache_ptr + offset_7, mask=mask_5, other=0.0)
        ...
        tl.store(C_exp_ptr + offset_8, tl.exp(tl.dot(Q, temp_9).to(tl.float16).to(tl.float32)).to(tl.float16), mask=mask_6)
    # Sequential loop p from 0 to 1040 with tile size BLOCK_P
    for p in range(0, 1040, BLOCK_P):
        ...
        C_sum = (tl.sum(temp_10, axis=2) + (C_sum * 1).to(tl.float16)).to(tl.float16)
    # Sequential loop p from 0 to 1040 with tile size BLOCK_P
    for p in range(0, 1040, BLOCK_P):
        ...
        C_div = (temp_11 / C_sum[:, :, None]).to(tl.float16)
        ...
        temp_12 = tl.load(V_cache_ptr + offset_11, mask=mask_9, other=0.0)
        O = ((O * 1).to(tl.float16) + tl.dot(C_div, temp_12).to(tl.float16)).to(tl.float16)
        offset_12 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None] * C_out1_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * C_out1_stride1
        mask_10 = (p_indices < P+M)[None, :]
        tl.store(C_out1_ptr + offset_12, tl.sum(C_div, axis=1), mask=mask_10)
        offset_13 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None] * C_out2_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * C_out2_stride1
        mask_11 = (p_indices < P+M)[None, :]
        tl.store(C_out2_ptr + offset_13, tl.sum((C_div * C_div).to(tl.float16), axis=1, dtype=tl.float16), mask=mask_11)
    temp_13 = tl.permute(O, (1, 0, 2))
    offset_14 = (tl.arange(0, 16))[:, None] * O2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * O2_stride1
    n_indices = n + tl.arange(0, BLOCK_N)
    mask_12 = (n_indices < N)[None, :]
    tl.store(O2_ptr + offset_14, tl.reshape(temp_13, (M, D)), mask=mask_12)
