...
def kernel_0(
    ...
    C_exp = tl.zeros((1, M, BLOCK_P), dtype=tl.float16)
    ...
    for k in range(0, 4544, BLOCK_K):
        ...
        X2 = (tl.sum((temp_0 * temp_0).to(tl.float16), axis=1, dtype=tl.float16) + (1 * X2).to(tl.float16)).to(tl.float16)
    ...
    for p in range(0, 1040, BLOCK_P):
        ...
        C_exp = tl.exp(tl.dot(Q_norm, temp_8).to(tl.float16).to(tl.float32)).to(tl.float16)
        C_sum = (tl.sum(C_exp, axis=2) + (1 * C_sum).to(tl.float16)).to(tl.float16)
        ...
        O = ((O * 1).to(tl.float16) + tl.dot(C_exp, temp_9).to(tl.float16)).to(tl.float16)
    # Skipped empty sloop with dummy body
    ...
    tl.store(O2_ptr + offset_8, tl.reshape(temp_10, (M, D)), mask=mask_6)
