...
def kernel_0(
    ...
    offset_8 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * O_stride0 + (tl.arange(0, 16))[None, :, None] * O_stride1 + (tl.arange(0, 64))[None, None, :] * O_stride2
    tl.store(O_ptr + offset_8, O.to(tl.float16))
...
def kernel_1(
    ...
    tl.store(O2_ptr + offset_1, tl.reshape(temp_1, (M, D)), mask=mask_6)


# Metadata for benchmark.py
TENSOR_PARAMS = ['K_cache', 'O', 'O2', 'V_cache', 'WK', 'WQ', 'WV', 'X']
BLOCK_PARAMS = ['block_k', 'block_p']

def forward(K_cache, O, O2, V_cache, WK, WQ, WV, X, block_k=16, block_p=16):
    """
