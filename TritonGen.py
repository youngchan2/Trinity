    ...
    def _generate_kernel_accumulator_init(self) -> str:
        ...
        for tensor in sorted(self.kernel_accumulators):
            ...
            if tensor in self.intermediate_tensor_indices:
                ...
                dtype = "tl.float32" if tensor in exp_tensors else "tl.float32"
            ...
            if tensor in self.intermediate_tensor_indices:
                ...
                if mask_var:
                    ...
                    if tensor in self.tensor_shapes:
                        ...
                        exp_tensors = getattr(self, 'exp_tensors', set())
                        store_value = f"{tensor}.to(tl.float32)" if tensor in exp_tensors else tensor
        ...
        for tensor_name in sorted(local_intermediates):
            if tensor_name in self.tensor_shapes:
                ...
                if tensor_name in accumulators or tensor_name in cross_sloop_tensors or tensor_name in sloop_intermediate_tensors:
                    ...
                    dtype = "tl.float32" if tensor_name in exp_tensors else "tl.float32"
...
"""
    ...
    def _generate_store(self, node: ASTNode) -> str:
        ...
        if val_node.node_type in [NodeType.TRANSPOSE, NodeType.PERMUTE3, NodeType.SQUEEZE, NodeType.UNSQUEEZE]:
            ...
            if val_node.node_type == NodeType.PERMUTE3:
                ...
                if not hasattr(child, 'temp_var'):
                    # Check if child contains loads or reduce operations
                    if self._contains_loads(child) or self._contains_reduce_sum(child):
                        # Generate all loads first
                        code += self._generate_loads_separately(child)

                        # Then generate the child expression using temp_vars from loads
                        child_expr = self._generate_node_without_loads(child)

                        # Create temp var for the expression
                        if child_expr:
                            temp_var_child = f"temp_{self.temp_counter}"
                            self.temp_counter += 1
                            indent = '    ' * self.indent_level
                            code += f"{indent}{temp_var_child} = {child_expr}\n"
                            child.temp_var = temp_var_child
                        else:
                            raise ValueError(f"Child node {child.node_type} has no temp_var and generates no expression")
                    # For UNSQUEEZE nodes, we need special handling to avoid inline generation
                    elif child.node_type == NodeType.UNSQUEEZE:
        ...
        # If storing to an exp_tensor (fp32), remove unnecessary .to(tl.float32) conversions
        exp_tensors = getattr(self, 'exp_tensors', set())
        if tensor_name in exp_tensors:
            # Remove .to(tl.float32) conversions for fp32 tensors
            value_expr = value_expr.replace('.to(tl.float32)', '')
    ...
    def _generate_binary_op(self, node: ASTNode, op: str) -> str:
        ...
        if node.node_type == NodeType.SQR:
            ...
            return f"({operand} * {operand}).to(tl.float32)"
        ...
        # Some nodes (LOAD, etc.) set temp_var and return "" (cached)
        # Others (NUM, VAR) return the value directly without temp_var
        left_expr = self._generate_node(left_child)
        if hasattr(left_child, 'temp_var'):
            left = left_child.temp_var
        elif left_expr:
            left = left_expr
        else:
            raise ValueError(f"No expression or temp_var for left operand")

        # Generate right operand
        right_expr = self._generate_node(right_child)
        if hasattr(right_child, 'temp_var'):
            right = right_child.temp_var
        elif right_expr:
            right = right_expr
        else:
            raise ValueError(f"No expression or temp_var for right operand")
        ...
        if current_tensor and current_tensor in exp_tensors:
            # Don't add .to(tl.float32) for fp32 tensors
            return f"({left} {op} {right})"
        else:
            return f"({left} {op} {right}).to(tl.float32)"
    
    def _generate_unary_op(self, node: ASTNode, op: str) -> str:
        """Generate unary operation"""
        child = node.children[0]

        # Generate operand (may set temp_var and return "", or return value directly)
        child_expr = self._generate_node(child)
        if hasattr(child, 'temp_var'):
            operand = child.temp_var
        elif child_expr:
            operand = child_expr
        else:
            raise ValueError(f"No expression or temp_var for unary op {op} operand")
        ...
        if op in ["tl.exp", "tl.sqrt", "tl.sigmoid"]:
            return f"{op}({operand}.to(tl.float32)).to(tl.float32)"
        else:
            # Don't add .to(tl.float32) for fp32 tensors
            if current_tensor and current_tensor in exp_tensors:
                return f"{op}({operand})"
            else:
                return f"{op}({operand}).to(tl.float32)"
    ...
    def _generate_matmul(self, node: ASTNode) -> str:
        ...
        if left_is_concat and right_is_concat:
            ...
            if left_axis == 1 and right_axis == 0:
                ...
                if len(left_tensors) == 2 and len(right_tensors) == 2:
                    ...
                    matmul1 = f"tl.dot({left_tensors[0]}, {right_tensors[0]}).to(tl.float32)"
                    matmul2 = f"tl.dot({left_tensors[1]}, {right_tensors[1]}).to(tl.float32)"
        ...
        # Generate left operand (may set temp_var and return "", or return value directly)
        left_expr = self._generate_node(left_child)
        if hasattr(left_child, 'temp_var'):
            left = left_child.temp_var
        elif left_expr:
            ...
            raise ValueError(f"No expression or temp_var for left matmul operand")
        ...
        if hasattr(right_child, 'temp_var'):
            ...
            raise ValueError(f"No expression or temp_var for right matmul operand")
        ...
        if current_tensor and current_tensor in exp_tensors:
            ...
            return f"tl.dot({left}, {right}).to(tl.float32)"
    ...
    def _generate_reduce_sum(self, node: ASTNode) -> str:
        ...
        # Generate the child expression (may set temp_var even if it returns "")
        tensor_expr = self._generate_node(child)

        # Use temp_var if available (e.g., from cached load), otherwise use the expression
        if hasattr(child, 'temp_var'):
            tensor = child.temp_var
        elif tensor_expr:
            tensor = tensor_expr
        else:
            raise ValueError(f"No tensor expression or temp_var for reduce_sum child")

        return f"tl.sum({tensor}, axis={axis}, dtype=tl.float32)"
    ...
    def _generate_broadcast(self, node: ASTNode) -> str:
        ...
        # Generate the child expression (may set temp_var even if it returns "")
        child_expr = self._generate_node(tensor_child)

        # Use temp_var if available (e.g., from cached load), otherwise use the expression
        if hasattr(tensor_child, 'temp_var'):
            tensor_expr = tensor_child.temp_var
        elif child_expr:
            tensor_expr = child_expr
        else:
            raise ValueError(f"No tensor expression or temp_var for broadcast child")
    ...
    def _generate_loads_and_nested_ops(self, node: ASTNode) -> str:
        ...
        for matmul_node in nested_matmuls:
            if matmul_node.children[1].node_type == NodeType.MATMUL:
                ...
                code += f"{indent_str}{temp_name} = tl.dot({left_expr}, {right_expr}).to(tl.float32)\n"
            ...
            if n.node_type == NodeType.MATMUL:
                ...
                if len(n.children) > 1 and n.children[1].node_type == NodeType.MATMUL:
                    ...
                    if not hasattr(inner_matmul, 'temp_var'):
                        ...
                        code_line = f"{indent_str}{temp_name} = tl.dot({left_expr}, {right_expr}).to(tl.float32)\n"
    ...
    def _generate_node_without_loads(self, node: ASTNode) -> str:
        ...
        if node.node_type == NodeType.LOAD:
            ...
            if hasattr(node, 'temp_var'):
                ...
                return f"tl.dot({left_expr}, {right_expr}).to(tl.float32)"
        elif node.node_type == NodeType.RSUM:
    ...
    def _generate_permute3(self, node: ASTNode) -> str:
        ...
        # First check if child contains loads or reduce operations
        # If so, generate loads separately first, then generate expression
        if self._contains_loads(child) or self._contains_reduce_sum(child):
            # Generate all loads first
            ...
            # If child doesn't have temp_var yet, create one for the expression
            if not hasattr(child, 'temp_var'):
                if child_expr:
                    temp_var_child = f"temp_{self.temp_counter}"
                    self.temp_counter += 1
                    indent = '    ' * self.indent_level
                    child_code += f"{indent}{temp_var_child} = {child_expr}\n"
                    child.temp_var = temp_var_child
                else:
                    raise ValueError(f"Child node {child.node_type} has no temp_var and generates no expression")
        else:
            # No loads, generate child directly
            child_code = self._generate_node(child)

            # If the child still doesn't have temp_var (e.g., for binary ops like MUL),
            # create a temporary variable for it
            if not hasattr(child, 'temp_var'):
                if child_code:
                    # child_code is an expression, assign it to a temp var
                    temp_var_child = f"temp_{self.temp_counter}"
                    self.temp_counter += 1
                    indent = '    ' * self.indent_level
                    child_code = f"{indent}{temp_var_child} = {child_code}\n"
                    child.temp_var = temp_var_child
                else:
                    # No code and no temp_var - this is an error
                    raise ValueError(f"Child node {child.node_type} has no temp_var and generates no code")
            elif child_code and not child_code.endswith('\n'):
        ...
        # Check if this permute3 was already generated (avoid duplicate generation)
        if hasattr(node, 'temp_var'):
            # Already generated, return empty string (temp_var is already set)
            return ""
    ...
    def _generate_squeeze(self, node: ASTNode) -> str:
        ...
        if source_tensor_name and source_tensor_name in self.tensor_shapes:
            # Use tensor dimension information from tensor_shapes
            ...
            # No source tensor name - use dynamic shape slicing
            # Build shape by getting all dimensions except the squeezed one
            code += f"{indent}# Squeeze dimension {dim} (dynamic)\n"
            # For 3D tensor: dim 0 → (shape[1], shape[2]), dim 1 → (shape[0], shape[2]), etc.
            # Assume 3D for now (can extend later)
            dim_int = int(dim)
            if dim_int == 0:
                shape_parts = [f"{tensor_expr}.shape[1]", f"{tensor_expr}.shape[2]"]
            elif dim_int == 1:
                shape_parts = [f"{tensor_expr}.shape[0]", f"{tensor_expr}.shape[2]"]
            elif dim_int == 2:
                shape_parts = [f"{tensor_expr}.shape[0]", f"{tensor_expr}.shape[1]"]
            else:
                # Fallback for higher dimensions
                code += f"{indent}# Warning: squeeze dim {dim} for >3D tensor\n"
                shape_parts = [f"{tensor_expr}.shape[{i}]" for i in range(3) if i != dim_int]
